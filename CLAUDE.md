...
- **Card tags**: `site/snippets/card-tags.php` (used by `feed-card.php` and `work-grid-card.php`) lists a card's services, sectors and editorial tags (`tags`, or `impactAreas` for case studies) as small blue links to `/our-work?filter=…`. The first four show; a "+N" button expands the rest (click handler in `service-page-scripts.php`). `.feed-wrap a` sets `color: inherit`, so tag colour rules need the `.feed-wrap` prefix.
