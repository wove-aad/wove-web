...
- **Layout**: a grid of `.team-card`s, 4 per row at 1280px+, 3 at 960px+, 2 at 600px+, 1 on phones. Each card shows photo (or initials), name and role with a +/− indicator. Clicking a card opens a full-width `.team-panel` directly below that card's row (the card stays in place) with the bio, a subtle LinkedIn link, the 3 latest Wove Mind entries credited to them and "See all {first name}'s posts →" linking to `/our-work?filter=author:{slug}`. One card is open at a time; JS moves the card's `.team-card__details` into the panel and re-seats the panel on resize. `/our-people#{slug}` opens that card on load (Our Work's "View profile" link uses this). Without JS the details show inside each card.
